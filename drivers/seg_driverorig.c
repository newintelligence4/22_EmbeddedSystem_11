...
static int driver_open(struct inode *device_file, struct file *instance){
    printk("seg_driver - open was called!\n");
    return 0;
}
...
static int driver_close(struct inode *device_file, struct file *instance){
    printk("seg_driver - close was called!\n");
    return 0;
}
...
static int __init ModuleInit(void) {
	printk("Hello, Kernel! by 7segment\n");
	...
	printk("seg) read_write - Device Nr. Major : %d, Minor : %d was registered!\n", my_device_nr >> 20, my_device_nr && 0xffffff);
...
static void __exit ModuleExit(void) {
	...
	class_destroy(my_class);
	unregister_chrdev_region(my_device_nr, 1);
	printk("Goodbye, Kernel by seg\b");
}
